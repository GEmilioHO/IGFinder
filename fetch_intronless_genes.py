...
single_exon_genes = {gene_id for gene_id, gene_info in genes.items() if gene_info['exons_n'] == 1 and gene_info['biotype'] == 'protein_coding'}
...
print(f"Of the {len(all_genes)} genes in {species}, "
      f"{len(multiple_exon_genes)} are multiple exon genes, "
      f"and only {len(single_exon_genes)} are single exon genes")
...
intronless_genes = {gene_id for gene_id in single_exon_genes if not gene_id in lst_introns_utr}

# --- Write the list of the single exon gene IDs
import csv

with open(f"../data/{species}-single_exon_genes.tsv", 'w', newline='') as segs_out, \
     open(f"../data/{species}-multiple_exon_genes.tsv", 'w', newline='') as megs_out, \
     open(f"../data/{species}-intronless_genes.tsv", 'w', newline='') as igs_out:

        fieldnames = fields + ['exons_n', 'transcripts_n']
